...
const Toggle = ({ checked, onChange, labelOn, labelOff, isDestructive }: {
  ...
  <button onClick={() => onChange(!checked)} title={checked ? labelOn : labelOff}>
    <div className="relative h-6 w-10 rounded-full transition-colors"
      style={{ background: checked ? (isDestructive ? T.brand : "#10B981") : "rgba(255,255,255,0.1)" }}>
...
const ProductDialog = ({ product, onSave, onClose }: {
  ...
  const inputStyle = { background: T.input, border: `1px solid ${T.border}`, color: T.text };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center font-pos" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)" }}>
      <div className="relative mx-4 w-full max-w-md rounded-xl p-6" style={{ background: T.card, border: `1px solid ${T.border}`, boxShadow: "0 25px 50px rgba(0,0,0,0.5)" }}>
        <button onClick={onClose} className="absolute right-4 top-4" style={{ color: T.textMuted }}>
          <X size={20} strokeWidth={2} />
        </button>
        <h3 className="mb-5 text-xl font-extrabold" style={{ color: T.text }}>
          {isNew ? "Agregar Producto" : "Editar Producto"}
        </h3>
        <div className="space-y-4">
          <div>
            <label className="mb-1 block text-[11px] font-bold uppercase tracking-wider" style={{ color: T.textDim }}>Nombre *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-[11px] font-bold uppercase tracking-wider" style={{ color: T.textDim }}>Precio *</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold" style={{ color: T.textMuted }}>$</span>
            ...
            <div>
              <label className="mb-1 block text-[11px] font-bold uppercase tracking-wider" style={{ color: T.textDim }}>Categoría</label>
          ...
          <div>
            <label className="mb-1 block text-[11px] font-bold uppercase tracking-wider" style={{ color: T.textDim }}>Descripción (opcional)</label>
        ...
        <div className="mt-6 flex gap-3">
          <button onClick={onClose} className="flex-1 rounded-lg py-3 text-sm font-bold transition-all duration-150"
            style={{ background: "rgba(255,255,255,0.06)", color: T.textMuted, border: `1px solid ${T.border}` }}>Cancelar</button>
...
const DeleteDialog = ({ product, onConfirm, onClose }: { product: Product; onConfirm: () => void; onClose: () => void; }) => (
  <div className="fixed inset-0 z-[100] flex items-center justify-center font-pos" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)" }}>
    <div className="relative mx-4 w-full max-w-sm rounded-xl p-6" style={{ background: T.card, border: `1px solid ${T.border}`, boxShadow: "0 25px 50px rgba(0,0,0,0.5)" }}>
      <h3 className="mb-2 text-xl font-extrabold" style={{ color: T.text }}>¿Eliminar producto?</h3>
      <p className="mb-1 text-sm" style={{ color: T.textMuted }}>Estás a punto de eliminar <strong style={{ color: T.text }}>{product.name}</strong>.</p>
      <p className="mb-5 text-xs" style={{ color: T.textDim }}>Esta acción no se puede deshacer.</p>
      <div className="flex gap-3">
        ...
        <button onClick={onConfirm} className="flex-1 rounded-lg py-3 text-sm font-bold text-white hover:brightness-110"
          style={{ background: T.brand }}>Eliminar</button>
      </div>
    </div>
  </div>
);
  ...
  return (
    <div className="space-y-4 font-pos">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-2xl font-extrabold" style={{ color: T.text }}>Productos</h2>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="relative flex-1">
          <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2" style={{ color: T.textDim }} />
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          <button onClick={() => setCatFilter("todos")}
            ...
            style={{ background: catFilter === "todos" ? "rgba(212,43,43,0.15)" : "rgba(255,255,255,0.05)", color: catFilter === "todos" ? "#F87171" : T.textMuted, border: `1px solid ${catFilter === "todos" ? "rgba(212,43,43,0.3)" : T.border}` }}>
            Todos
          </button>
          {categories.map((c) => (
            <button key={c.id} onClick={() => setCatFilter(c.id)}
              className="whitespace-nowrap rounded-lg px-4 py-2 text-sm font-bold transition-all duration-150"
              style={{ background: catFilter === c.id ? "rgba(212,43,43,0.15)" : "rgba(255,255,255,0.05)", color: catFilter === c.id ? "#F87171" : T.textMuted, border: `1px solid ${catFilter === c.id ? "rgba(212,43,43,0.3)" : T.border}` }}>
              {c.icon} {c.name}
            </button>
          ))}
        </div>
      ...
      <p className="text-xs font-medium" style={{ color: T.textDim }}>{filtered.length} producto{filtered.length !== 1 ? "s" : ""}</p>

      <div className="overflow-x-auto rounded-xl" style={{ background: T.card, border: `1px solid ${T.border}` }}>
            ...
            {filtered.map((product) => {
              ...
              return (
                <tr key={product.id} className={cn("transition-opacity", (!product.active || product.soldOut) && "opacity-40")}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="truncate font-bold" style={{ color: T.text }}>{product.name}</p>
                        {product.description && <p className="truncate text-xs" style={{ color: T.textDim }}>{product.description}</p>}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right font-bold" style={{ color: T.text }}>${product.price}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => setEditProduct(product)} className="rounded-lg p-2 transition-colors"
                        style={{ color: T.textMuted }} onMouseEnter={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.06)"}
                        onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                        <Pencil size={16} strokeWidth={2} />
                      </button>
                      <button onClick={() => setDeleteTarget(product)} className="rounded-lg p-2 transition-colors"
                        style={{ color: T.textMuted }} onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(212,43,43,0.1)"; e.currentTarget.style.color = "#F87171"; }}
        ...
        {filtered.length === 0 && <div className="py-12 text-center text-sm" style={{ color: T.textDim }}>No se encontraron productos</div>}
      </div>
